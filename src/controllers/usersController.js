...
const userList = async (req, res) => {
    try {
        const users = await User.findAll({ attributes: ['id', 'name', 'email'], include: [{ model: ShoppingList, as: 'shoppingLists', attributes: ['id', 'name', 'product_list', 'total_discount', 'final_price'] }] });
        res.json(users);
    } catch (error) {
